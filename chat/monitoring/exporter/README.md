# Sunrise Metric Exporter

This is a simple service which reads JSON monitoring data exposed by Sunrise server using [expvar](https://golang.org/pkg/expvar/) and re-publishes it in other formats. Currently the supported formats are:
...
Exporters are intended to run next to (pair with) Sunrise servers: one Exporter per one Sunrise server, i.e. a single Exporter provides metrics from a single Sunrise server. 
...
* `sunrise_addr` is the address where the Sunrise instance publishes `expvar` data to scrape; default: `http://localhost:6060/stats/expvar`.
...
./exporter \
    --serve_for=influxdb \
    --sunrise_addr=http://localhost:6060/stats/expvar \
    --listen_at=:6222 \
    --instance=exp-0 \
...
### Prometheus
* `prom_namespace` is a prefix to use for metrics names. If you are monitoring multiple sunrise instances you may want to use different namespaces; default: `sunrise`.
* `prom_metrics_path` is the path under which to expose the metrics for scraping; default: `/metrics`.
* `prom_timeout` is the Sunrise connection timeout in seconds in response to Prometheus scrapes; default: `15`.
