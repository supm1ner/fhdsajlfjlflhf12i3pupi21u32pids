...
func main() {
	log.Printf("Sunrise metrics exporter.")

	var (
		serveFor = flag.String("serve_for", "influxdb",
			"Monitoring service to gather metrics for. Available: influxdb, prometheus.")
		sunriseAddr = flag.String("sunrise_addr", "http://localhost:6060/stats/expvar",
			"Address of the Sunrise instance to scrape.")
		...
		promNamespace   = flag.String("prom_namespace", "sunrise", "Prometheus namespace for metrics '<namespace>_...'")
		promMetricsPath = flag.String("prom_metrics_path", "/metrics", "Path under which to expose metrics for Prometheus scrapes.")
		promTimeout     = flag.Int("prom_timeout", 15, "Sunrise connection timeout in seconds in response to Prometheus scrapes.")
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`<html><head><title>Sunrise Exporter</title></head><body>
<h1>Sunrise Exporter</h1>
...
<pre>` + version.Info() + ` ` + version.BuildContext() + `</pre>
	...
	scraper := Scraper{address: *sunriseAddr, simpleMetrics: metrics, histogramMetrics: histoMetrics}
	var serverTypeString string
	// Create exporters.
	switch service {
	case promService:
		serverTypeString = *serveFor
		promExporter := NewPromExporter(*sunriseAddr, *promNamespace, time.Duration(*promTimeout)*time.Second, &scraper)
	...
	case influxService:
		...
		http.HandleFunc("/push", func(w http.ResponseWriter, r *http.Request) {
			...
			w.Write([]byte(`<html><head><title>Sunrise Push</title></head><body>
<h1>Sunrise Push</h1>
<pre>` + msg + `</pre>
</body></html>`))
		})
	...
	log.Println("Reading Sunrise expvar from", *sunriseAddr)
