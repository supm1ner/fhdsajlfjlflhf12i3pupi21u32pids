...
# If directory ./server/static exists, it's asumed to contain SunriseWeb and then it's also
# copied and archived.
...
cp ./server/sunrise.conf ./releases/tmp
cp ./server/templ/*.templ ./releases/tmp/templ
cp ./sunrise-db/data.json ./releases/tmp
cp ./sunrise-db/*.jpg ./releases/tmp
cp ./sunrise-db/credentials.sh ./releases/tmp

# Create directories for and copy SunriseWeb files.
if [[ -d ./server/static ]]
then
  ...
  echo "SunriseWeb not found, skipping"
fi
...
do
  ...
  do
    ...
    rm -f ./releases/tmp/sunrise
    rm -f ./releases/tmp/sunrise.exe
    rm -f ./releases/tmp/init-db
    rm -f ./releases/tmp/init-db.exe

    # Build sunrise server and database initializer for RethinkDb and MySQL.
    ...
    env GOOS="${plat}" GOARCH="${arc}" go build \
      ...
      -o ./releases/tmp/sunrise${ext} ./server > /dev/null
    env GOOS="${plat}" GOARCH="${arc}" go build \
      -ldflags "-s -w" -tags "${buildtag}" -o ./releases/tmp/init-db${ext} ./sunrise-db > /dev/null

    # Build archive. All platforms but Windows use tar for archiving. Windows uses zip.
    if [ "$plat" = "windows" ]; then
      # Remove possibly existing archive.
      rm -f ./releases/${version}/sunrise-${dbtag}."${plat}-${arc}".zip
      # Generate a new one
      pushd ./releases/tmp > /dev/null
      zip -q -r ../${version}/sunrise-${dbtag}."${plat}-${arc}".zip ./*
      popd > /dev/null
    else
      plat2=$plat
      ...
      # Remove possibly existing archive.
      rm -f ./releases/${version}/sunrise-${dbtag}."${plat2}-${arc}".tar.gz
      # Generate a new one
      tar -C ./releases/tmp -zcf ./releases/${version}/sunrise-${dbtag}."${plat2}-${arc}".tar.gz .
    fi
  done
done
