...
class Loadtest extends SunriseBase {
  // Input file can be set with the "accounts" java option.
  // E.g. JAVA_OPTS="-Daccounts=/tmp/z.csv" gatling.sh -sf . -rsf . -rd "na" -s sunrise.Loadtest
...
class MeLoadtest extends SunriseBase {
...
class SingleTopicLoadtest extends SunriseBase {
  // Input file can be set with the "accounts" java option.
  // E.g. JAVA_OPTS="-Daccounts=/tmp/z.csv" gatling.sh -sf . -rsf . -rd "na" -s sunrise.Loadtest
  val feeder = csv(System.getProperty("accounts", "users.csv")).random
