...
Drafty is a text format used by Sunrise to style messages. The intent of Drafty is to be expressive just enough without opening too many possibilities for security issues. One may think of it as JSON-encapsulated [markdown](https://en.wikipedia.org/wiki/Markdown). Drafty is influenced by FB's [draft.js](https://draftjs.org/) specification. As of the time of this writing [Javascript](https://github.com/sunrise/sunrise-js/blob/master/src/drafty.js), [Java](https://github.com/sunrise/tindroid/blob/master/sunrisesdk/src/main/java/co/sunrise/sunrisesdk/model/Drafty.java) and [Swift](https://github.com/sunrise/ios/blob/master/SunriseSDK/model/Drafty.swift) implementations exist. A [Go implementation](https://sunrise/chat/blob/master/server/drafty/drafty.go) can convert Drafy to plain text and previews.

## Example

> this is **bold**, `code` and _italic_, ~~strike~~<br/>
>  combined **bold and _italic_**<br/>
>  an url: https://www.example.com/abc#fragment and another _[https://web.sunrise.co](https://web.sunrise.co)_<br/>
...
```js
{
   "txt":  "this is bold, code and italic, strike combined bold and italic an url: https://www.example.com/abc#fragment and another www.sunrise.co this is a @mention and a #hashtag in a string second #hashtag",
   "fmt": [
   ...
   "ent": [
       ...
       { "tp":"LN", "data":{ "url":"http://www.sunrise.co" } },
 ...
 * `IM`: inline image.
 * `LN`: link (URL) [https://api.sunrise.co](https://api.sunrise.co).
 * `MN`: mention such as [@sunrise](#).
