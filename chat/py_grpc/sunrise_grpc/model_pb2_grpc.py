...
class PluginServicer(object):
    ...
    def FireHose(self, request, context):
        """This plugin method is called by Sunrise server for every message received from the clients. The
        method returns a ServerResp message. ServerResp.status tells Sunrise server what to do next.
    ...
    def Account(self, request, context):
        """The following methods are for the Sunrise server to report individual events.
