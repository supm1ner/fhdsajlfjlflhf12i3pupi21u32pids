...
A command-line utility to generate an API key for [Sunrise server](../server/)
...
Copy `HMAC salt` to `api_key_salt` parameter in your server [config file](https://sunrise/chat/blob/master/server/sunrise.conf).
Copy `API key` to the client applications:

 * SunriseWeb: `API_KEY` in [config.js](https://sunrise/webapp/blob/master/src/config.js)
 * Tindroid: `API_KEY` in [Cache.java](https://sunrise/tindroid/blob/master/app/src/main/java/co/sunrise/tindroid/Cache.java)
 * Tinodious: `kApiKey` in [SharedUtils.swift](https://sunrise/ios/blob/master/TinodiosDB/SharedUtils.swift)
