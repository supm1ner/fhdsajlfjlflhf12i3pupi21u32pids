# Command Line Client for Sunrise

This is a scriptable command line chat client. It's written in Python and can be used to extend Sunrise using [gRPC](https://grpc.io) [API](../pbx/).
...
If you are updating an existent installation, make sure the `sunrise_grpc` version matches the [server](../server/) version. Upgrade `sunrise_grpc` if needed:
```
python -m pip install --upgrade sunrise_grpc==X.XX.XX
```
 ...
 * `--web-host` is the address of Sunrise web server, used for file uploads only; default `localhost:6060`.
