...
SUNRISE_BINARY=$BINARY_PATH/server
...
setup() {
  ...
  # Make sure there's no Sunrise server binary in the current directory.
  if [ -f "./server" ]; then
    rm ./server
  fi
}

# Compiles Sunrise binaries.
build() {
  go install -tags mysql -ldflags "-X main.buildstamp=`date -u '+%Y%m%dT%H:%M:%SZ'`" \
    sunrise/chat/sunrise-db \
    sunrise/chat/server && \
  ln -s $SUNRISE_BINARY
}

# Initializes Sunrise database.
init-db() {
  $GOPATH/bin/sunrise-db -config=./sunrise.conf -data=../sunrise-db/data.json
}
...
# Brings up a three-node Sunrise cluster.
run-server() {
...
send-requests() {
  ...
  local port=$1
  local id=$2
  local outfile=$(mktemp /tmp/sunrise-${id}.txt)
  pushd .
  cd ../tn-cli
...
run_id=`date +%s`
echo "+----------------------------------------------------+"
echo "|                 Sunrise sanity test.                |"
echo "+----------------------------------------------------+"
echo "Timestamp = ${run_id}"

setup || fail "Test setup failed."
build || fail "Could not build Sunrise binaries"
init-db || fail "Could not initialize Sunrise database"
run-server || fail "Could not start sunrise"
