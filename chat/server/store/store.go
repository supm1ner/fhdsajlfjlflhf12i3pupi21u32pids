...
import (
	...
	adapter "sunrise/chat/server/db"
	"sunrise/chat/server/logs"
	"sunrise/chat/server/media"
	"sunrise/chat/server/store/types"
	"sunrise/chat/server/validate"
)
...
func openAdapter(workerId int, jsonconf json.RawMessage) error {
	...
	if adp == nil {
		if len(config.UseAdapter) > 0 {
			...
			return errors.New("store: db adapter is not specified. Please set `store_config.use_adapter` in `sunrise.conf`")
		}
	}
