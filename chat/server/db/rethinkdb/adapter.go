...
import (
	...
	"sunrise/chat/server/db/common"
	"sunrise/chat/server/logs"
	"sunrise/chat/server/store"
	t "sunrise/chat/server/store/types"
...
func (a *adapter) topicStateForUser(uid t.Uid, now time.Time, update any) error {
	...
	/*
		r.db('sunrise').table('topics').getAll(
			r.args(
				r.db("sunrise").table("subscriptions").getAll('S8VFqRpXw5M', {index: 'User'})('Topic').coerceTo('array')
			)
		).update(...)
	*/
	if _, err := rdb.DB(a.dbName).Table("topics").
...
func (a *adapter) UserUnreadCount(ids ...t.Uid) (map[t.Uid]int, error) {
	...
	/*
		Query:
			r.db("sunrise").table("subscriptions").getAll("356zaYaumiU", "k4cvfaq8zCQ", {index: "User"})
			  .eqJoin("Topic", r.db("sunrise").table("topics"), {index: "Id"})
			  .filter(
...
func (a *adapter) UserGetUnvalidated(lastUpdatedBefore time.Time, limit int) ([]t.Uid, error) {
	/*
		Query:
			r.db('sunrise').table('users')
				.filter(r.row('LastSeen').eq(null).and(r.row('UpdatedAt').lt('Mar 31 2022 01:03:38')))
				.eqJoin('Id', r.db('sunrise').table('credentials'), {index: 'User'}).zip()
				.pluck('User', 'Done')
				.group('User')
...
func (a *adapter) Find(caller, promoPrefix string, req [][]string, opt []string, activeOnly bool) ([]t.Subscription, error) {
	...
	/*
		r.db('sunrise').
			...
			union(r.db('sunrise').table('topics').getAll('basic:alice', 'travel', {index: "Tags"})).
