...
func (s *Session) dispatch(msg *ClientComMessage) {
	...
	if msg.Extra == nil || (msg.Extra.AsUser == "" && auth.ParseAuthLevel(msg.Extra.AuthLevel) == auth.LevelNone) {
		...
		if asLvl := auth.ParseAuthLevel(msg.Extra.AuthLevel); asLvl == auth.LevelNone {
			// AuthLvl is not set by the caller, assign default LevelAuth.
			msg.AuthLvl = int(auth.LevelAuth)
		} else {
			msg.AuthLvl = int(asLvl)
		}
	}
