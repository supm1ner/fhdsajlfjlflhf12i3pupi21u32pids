...
func platformFromUA(ua string) string {
	...
	case strings.Contains(ua, "reactnative"):
		...
		return ""
	case strings.Contains(ua, "sunrisejs"):
		return "web"
...
// if addr is in the form "unix:/run/sunrise.sock" it's a unix socket, otherwise TCP host:port.
...
// Check if specified address is a unix socket like "unix:/run/sunrise.sock".
