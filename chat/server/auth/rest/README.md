...
This authenticator permits authentication of Sunrise users and creation of Sunrise accounts using a separate process as a
source of truth. For instance, if accounts are managed by corporate LDAP, this service allows handling of Sunrise
...
- [REST or JSON-RPC authenticator](#rest-or-json-rpc-authenticator)
	...
	- [The server must implement the following endpoints:](#the-server-must-implement-the-following-endpoints)
		...
		- [`auth` Request for authentication](#auth-request-for-authentication)
			...
			- [Sample response when the account needs to be created by Sunrise](#sample-response-when-the-account-needs-to-be-created-by-sunrise)
...
Add the following section to the `auth_config` in [sunrise.conf](../../sunrise.conf):
...
Request to authenticate a user. Client (Sunrise) provides a secret, authentication server responds with a user record.
If this is a very first login and the server manages the accounts, the server may return `newacc` object which will
be used by client (Sunrise) to create the account. The server may optionally return a challenge as `byteval`.
...
#### Sample response when the account needs to be created by Sunrise
```js
{
  "rec": {
...
If server requested Sunrise to create a new account, this endpoint is used to link the new Sunrise user ID with the
...
These are also used in Sunrise discovery mechanism (e.g. searching for users, contact sync). See
