...
USAGE="Usage: $0 [ --config <path_to_sunrise.conf> ] {start|stop}"
...
while [[ $# -gt 0 ]]; do
  ...
  case "$key" in
    ...
    start)
      ...
      do
        # Start the node
        $SERVER -config=${SUNRISE_CONF} -cluster_self=${NODE_NAME} -listen=:${HTTP_PORT} -grpc_listen=:${GRPC_PORT} -static_data=${STATIC_DATA_DIR} -log_flags=stdFlags,shortfile &
        # Save PID of the node to a temp file.
        # /var/tmp/ does not requre root access.
        echo $!> "/var/tmp/sunrise-${NODE_NAME}.pid"
    ...
    stop)
      ...
      do
        ...
        kill `cat /var/tmp/sunrise-${NODE_NAME}.pid`
        # Clean up: delete temp files.
        rm "/var/tmp/sunrise-${NODE_NAME}.pid"
      done
      exit 0
      ;;
    *)
