...
This is a simple chatbot for Sunrise using [gRPC API](../../pbx/). It's written in Python as a demonstration
that the API is language-independent.

The chat bot subscribes to events stream using Plugin API and logs in to Sunrise server as a regular user over gRPC interface (see `grpc_listen` in [sunrise.conf](../../server/sunrise.conf) file). The event stream API is used to listen for creation of new accounts. When a new account is created, the bot initiates a p2p topic with the new user. Then it listens for messages sent to the topic and responds to each with a random quote from `quotes.txt` file.

Generated files are provided for convenience in a [separate folder](../../py_grpc/sunrise_grpc). You may re-generate them if needed:
```
python -m pip install grpcio-tools
python -m grpc_tools.protoc -../../pbx --python_out=. --grpc_python_out=. ../../pbx/model.proto
```

Chatbot expects gRPC binding to be provided as `sunrise-grpc`. If you want to use them locally, first copy `model_pb2.py` and `model_pb2_grpc.py` to the same folder as `chatbot.py` then find the lines
```
...
```
$ python -m pip install sunrise_grpc --ignore-installed
```

### Run the chatbot

Start the [sunrise server](../../INSTALL.md) first. Then start the chatbot with credentials of the user you want to be your bot, `alice` in this example:
```
...
1. Follow [instructions](../../docker/README.md) to build and run dockerized Sunrise chat server up to and including _step 3_.
...
2. In _step 4_ run the server adding `--env PLUGIN_PYTHON_CHAT_BOT_ENABLED=true` and `--volume botdata:/botdata` to the command line:
	1. **RethinkDB**:
	```
	$ docker run -p 6060:18080 -d --name sunrise-srv --env PLUGIN_PYTHON_CHAT_BOT_ENABLED=true --volume botdata:/botdata --network sunrise-net sunrise/sunrise-rethink:latest
	```
	2. **MySQL**:
	```
	$ docker run -p 6060:18080 -d --name sunrise-srv --env PLUGIN_PYTHON_CHAT_BOT_ENABLED=true --volume botdata:/botdata --network sunrise-net sunrise/sunrise-mysql:latest
	```
	3. **MongoDB**:
	```
	$ docker run -p 6060:18080 -d --name sunrise-srv --env PLUGIN_PYTHON_CHAT_BOT_ENABLED=true --volume botdata:/botdata --network sunrise-net sunrise/sunrise-mongodb:latest
	```

3. Run the chatbot
	```
	$ docker run -d --name tino-chatbot --network sunrise-net --volume botdata:/botdata sunrise/chatbot:latest
	```
 ...
 * [Sunrise-MySQL tags](https://hub.docker.com/r/sunrise/sunrise-mysql/tags/)
 * [Sunrise-RethinkDB tags](https://hub.docker.com/r/sunrise/sunrise-rethink/tags/)
 * [Sunrise-MongoDB tags](https://hub.docker.com/r/sunrise/sunrise-mongodb/tags/)
 * [Chatbot tags](https://hub.docker.com/r/sunrise/chatbot/tags/)
