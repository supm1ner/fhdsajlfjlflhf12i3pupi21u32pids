...
These reference docker-compose files will run Sunrise with the MySql backend either as [a single-instance](single-instance.yml) or [a 3-node cluster](cluster.yml) setup.

```
docker-compose -f <name of the file> [-f <name of override file>] up -d
```

By default, this command starts up a mysql instance, Sunrise server(s) and Sunrise exporter(s).
Sunrise server(s) is(are) configured similar to [Sunrise Demo/Sandbox](../../README.md#demosandbox) and
maps its web port to the host's port 6060 (6061, 6062). Sunrise exporter(s) serve(s) metrics for InfluxDB.
...
E.g. to start the Sunrise server in the single-instance MySql setup,
```
docker-compose -f single-instance.yml up -d sunrise-0
```

### Database resets and/or version upgrades
To reset the database or upgrade the database version, you can set `RESET_DB` or `UPGRADE_DB` environment variable to true when starting Sunrise with docker-compose.
E.g. for upgrading the database in MongoDb cluster setup, use:
```
UPGRADE_DB=true docker-compose -f cluster.yml -f cluster.mongodb.yml up -d sunrise-0
```

For resetting the database in RethinkDb single-instance setup, run:
```
RESET_DB=true docker-compose -f single-instance.yml -f single-instance.rethinkdb.yml up -d sunrise-0
```
...
If the Sunrise server(s) are failing, you can print the job's stdout/stderr with:
```
docker logs sunrise-<instance number>
```

Additionally, you can examine the jobs `sunrise.log` file. To download it from the container, run:
```
docker cp sunrise-<instance number>:/var/log/sunrise.log .
```
