...
# Required env vars.
common_vars=( SUNRISE_ADDR INSTANCE SERVE_FOR )
...
# Common arguments.
args=("--sunrise_addr=${SUNRISE_ADDR}" "--instance=${INSTANCE}" "--listen_at=${LISTEN_AT}" "--serve_for=${SERVE_FOR}")
...
# Wait for Sunrise server if needed.
if [ ! -z "$WAIT_FOR" ] ; then
