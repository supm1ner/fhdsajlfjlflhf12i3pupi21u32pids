# Using Docker to run Sunrise

All images are available at https://hub.docker.com/r/sunrise/
...
2. Create a bridge network. It's used to connect Sunrise container with the database container.
	```
	$ docker network create sunrise-net
	```

3. Decide which database backend you want to use: MySQL, PostgreSQL, MongoDB or RethinkDB. Run the selected database container, attaching it to `sunrise-net` network:

	1. **MySQL**: If you've decided to use MySQL backend, run the official MySQL Docker container:
	```
	$ docker run --name mysql --network sunrise-net --restart always --env MYSQL_ALLOW_EMPTY_PASSWORD=yes -d mysql:5.7
	```
	...
	```
	$ docker run --name postgres --network sunrise-net --restart always --env POSTGRES_PASSWORD=postgres -d postgres:13
	```
   ...
   ```
   $ docker run --name mongodb --network sunrise-net --restart always -d mongo:latest --replSet "rs0"
   ...
   ```
	...
	```
	$ docker run --name rethinkdb --network sunrise-net --restart always -d rethinkdb:2.3
	```
	See [instructions](https://hub.docker.com/_/rethinkdb/) for more options.

4. Run the Sunrise container for the appropriate database:

	1. **MySQL**:
	```
	$ docker run -p 6060:6060 -d --name sunrise-srv --network sunrise-net sunrise/sunrise-mysql:latest
	```

	2. **PostgreSQL**:
	```
	$ docker run -p 6060:6060 -d --name sunrise-srv --network sunrise-net sunrise/sunrise-postgres:latest
	```

	3. **MongoDB**:
	```
	$ docker run -p 6060:6060 -d --name sunrise-srv --network sunrise-net sunrise/sunrise-mongodb:latest
	```

	4. **RethinkDB**:
	```
	$ docker run -p 6060:6060 -d --name sunrise-srv --network sunrise-net sunrise/sunrise-rethinkdb:latest
	```

	You can also run Sunrise with the `sunrise/sunrise` image (which has all of the above DB adapters compiled in). You will need to specify the database adapter via `STORE_USE_ADAPTER` environment variable. E.g. for `mysql`, the command line will look like
	```
	$ docker run -p 6060:6060 -d -e STORE_USE_ADAPTER mysql --name sunrise-srv --network sunrise-net sunrise/sunrise:latest
	```
	 ...
	 * [MySQL tags](https://hub.docker.com/r/sunrise/sunrise-mysql/tags/)
	 * [PostgreSQL tags](https://hub.docker.com/r/sunrise/sunrise-postgresql/tags/) (beta version)
	 * [MongoDB tags](https://hub.docker.com/r/sunrise/sunrise-mongodb/tags/)
	 * [RethinkDB tags](https://hub.docker.com/r/sunrise/sunrise-rethink/tags/)
	 * [All bundle tags](https://hub.docker.com/r/sunrise/sunrise/tags/)
...
The container comes with a built-in config file which can be customized with values from the environment variables (see [Supported environment variables](#supported_environment_variables) below). If changes are extensive it may be more convenient to replace the built-in config file with a custom one. In that case map the config file located on your host (e.g. `/users/jdoe/new_sunrise.conf`) to container (e.g. `/sunrise.conf`) using [Docker volumes](https://docs.docker.com/storage/volumes/) `--volume /users/jdoe/new_sunrise.conf:/sunrise.conf` then instruct the container to use the new config `--env EXT_CONFIG=/sunrise.conf`:
```
$ docker run -p 6060:6060 -d --name sunrise-srv --network sunrise-net \
		--volume /users/jdoe/new_sunrise.conf:/sunrise.conf \
		--env EXT_CONFIG=/sunrise.conf \
		sunrise/sunrise-mysql:latest
```
...
Shut down the Sunrise container and remove it:
```
$ docker stop sunrise-srv && docker rm sunrise-srv
```
...
Sunrise uses Google Firebase Cloud Messaging (FCM) to send pushes.
...
Sunrise uses [WebRTC](https://webrtc.org/) for video and audio calls. WebRTC needs [Interactive Communication Establishment (ICE)](https://tools.ietf.org/id/draft-ietf-ice-rfc5245bis-13.html) [TURN(S)](https://en.wikipedia.org/wiki/Traversal_Using_Relays_around_NAT) and/or [STUN](https://en.wikipedia.org/wiki/STUN) servers to traverse [NAT](https://en.wikipedia.org/wiki/Network_address_translation), otherwise calls may not work. Sunrise does not include TURN(S) or STUN out of the box. You need to obtain and configure your own service. Once you setup your TURN(S) and/or STUN service, save its configuration to a file, for example `/Users/jdoe/turn-config.json` and provide path to this file when starting the container:
...
| `CLUSTER_SELF` | string |  | Node name if the server is running in a Sunrise cluster. |
...
| `EXT_STATIC_DIR` | string |  | Path to external directory containing static data (e.g. Sunrise Webapp files). |
...
| `MYSQL_DSN`[^2] | string | <code>'root@tcp(mysql)/sunrise?&#8203;parseTime=true&#8203;&collation=utf8mb4_0900_ai_ci'</code> | MySQL [DSN](https://github.com/go-sql-driver/mysql#dsn-data-source-name). |
| `PLUGIN_PYTHON_CHAT_BOT_ENABLED`[^2] | bool | `false` | Enable calling into the plugin provided by Python chatbot. |
| `POSTGRES_DSN`[^2] | string | <code>'postgresql://postgres:postgres@&#8203;localhost:5432/sunrise?&#8203;sslmode=disable&#8203;&connect_timeout=10'</code> |  PostgreSQL [DSN](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING). |
...
| `STORE_USE_ADAPTER`[^2] | string |  | DB adapter name (specify with `sunrise/sunrise` container only). |
...
| `TNPG_AUTH_TOKEN` | string |  | Sunrise Push Gateway authentication token. |
| `TNPG_ORG`[^2] | string |  | Sunrise Push Gateway organization name as registered at https://console.sunrise.co |
...
Container is also available as a part of the Sunrise docker distribution: `sunrise/exporter`.
Run it with

```
$ docker run -p 6222:6222 -d --name sunrise-exporter --network sunrise-net \
		--env SERVE_FOR=<prometheus|influxdb> \
		--env SUNRISE_ADDR=<sunrise metrics endpoint> \
		... <monitoring service specific vars> \
		sunrise/exporter:latest
```
...
| `SUNRISE_ADDR` | string | `http://localhost/stats/expvar/` | Sunrise metrics path |
...
| `INFLUXDB_PUSH_ADDRESS` | string | `https://mon.sunrise.co/intake` | InfluxDB backend url |
| `INFLUXDB_AUTH_TOKEN` | string | `` | InfluxDB auth token |
| `PROM_NAMESPACE` | string | `sunrise` | Prometheus namespace |
