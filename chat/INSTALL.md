...
The config file [`sunrise.conf`](./server/sunrise.conf) contains extensive instructions on configuring the server.

## Installing from Binaries

1. Visit the [Releases page](https://sunrise/chat/releases/), choose the latest or otherwise the most suitable release. From the list of binaries download the one for your database (supported: MySQL, PostgreSQL, MongoDB, RethinkDB) and platform (Linux ARM or Intel, Windows, Mac ARM or Intel). Once the binary is downloaded, unpack it to a directory of your choosing, `cd` to that directory.

2. Make sure your database is running. Make sure it's configured to accept connections from `localhost`. In case of MySQL, Sunrise will try to connect as `root` without the password. In case of PostgreSQL, Sunrise will try connect as `postgres` with the password `postgres`. See notes below (_Building from Source_, section 4) on how to configure Sunrise to use a different user or a password. MySQL 5.7 or above is required (use InnoDB, not MyISAM storage engine). MySQL 5.6 or below **will not work**, use of MyISAM **will cause problems**. PostgreSQL 13 or above is required. PostgreSQL 12 or below **will not work**. MongoDB 4.4 or above is required. MongoDB 4.2 and below **will not work**.
...
4. Run the `sunrise` (or `sunrise.exe` on Windows) server. It will work without any parameters.
	```
	./sunrise
	```
...
4. Fetch, build Sunrise server and sunrise-db database initializer:
  - **MySQL**:
	```
	go install -tags mysql sunrise/chat/server@latest
	go install -tags mysql sunrise/chat/sunrise-db@latest
	```
  - **PostgreSQL**:
	```
	go install -tags postgres sunrise/chat/server@latest
	go install -tags postgres sunrise/chat/sunrise-db@latest
	```
  - **MongoDB**:
	```
	go install -tags mongodb sunrise/chat/server@latest
	go install -tags mongodb sunrise/chat/sunrise-db@latest
	```
  - **RethinkDb**:
	```
	go install -tags rethinkdb sunrise/chat/server@latest
	go install -tags rethinkdb sunrise/chat/sunrise-db@latest
	```
  - **All** (bundle all of the above DB adapters):
	```
	go install -tags "mysql rethinkdb mongodb postgres" sunrise/chat/server@latest
	go install -tags "mysql rethinkdb mongodb postgres" sunrise/chat/sunrise-db@latest
	```

    The steps above install Sunrise binaries at `$GOPATH/bin/`, sorces and supporting files are located at `$GOPATH/pkg/mod/sunrise/chat@vX.XX.X/` where `X.XX.X` is the version you installed, such as `0.19.1`.
    ...
    ```
    go install -tags mysql -ldflags "-X main.buildstamp=`date -u '+%Y%m%dT%H:%M:%SZ'`" sunrise/chat/server@latest
    ```
    The value of `buildstamp` will be sent by the server to the clients.

    Building with Go 1.17 or below **will fail**!

5. Open `sunrise.conf` (located at `$GOPATH/pkg/mod/sunrise/chat@vX.XX.X/server/`). Check that the database connection parameters are correct for your database. If you are using MySQL make sure [DSN](https://github.com/go-sql-driver/mysql#dsn-data-source-name) in `"mysql"` section is appropriate for your MySQL installation. Option `parseTime=true` is required.
```js
	"mysql": {
		"dsn": "root@tcp(localhost)/sunrise?parseTime=true",
		"database": "sunrise"
	},
```

6. Make sure you specify the adapter name in your `sunrise.conf`. E.g. you want to run Sunrise with MySQL:
```js
	"store_config": {
		...
		"use_adapter": "mysql",
...
If you followed instructions in the previous section then the Sunrise binaries are installed in `$GOPATH/bin/`, the sources and supporting files are located in `$GOPATH/pkg/mod/sunrise/chat@vX.XX.X/`, where `X.XX.X` is the version you installed, for example `0.19.1`.

Switch to sources directory (replace `X.XX.X` with your actual version, such as `0.19.1`):
```
cd $GOPATH/pkg/mod/sunrise/chat@vX.XX.X
```
...
2. Run DB initializer
	```
	$GOPATH/bin/sunrise-db -config=./sunrise-db/sunrise.conf
	```
	add `-data=./sunrise-db/data.json` flag if you want sample data to be loaded:
	```
	$GOPATH/bin/sunrise-db -config=./sunrise-db/sunrise.conf -data=./sunrise-db/data.json
	```

	DB initializer needs to be run only once per installation. See [instructions](sunrise-db/README.md) for more options.

3. Unpack JS client to a directory, for instance `$HOME/sunrise/webapp/` by unzipping `https://sunrise/webapp/archive/master.zip` and `https://sunrise/sunrise-js/archive/master.zip` to the same directory.
...
5. Run the server
	```
	$GOPATH/bin/server -config=./server/sunrise.conf -static_data=$HOME/sunrise/webapp/
	```
...
**Important!** If you are running Sunrise alongside another webserver, such as Apache or nginx, keep in mind that you need to launch the webapp from the URL served by Sunrise. Otherwise it won't work.
...
```
$GOPATH/bin/sunrise -config=./server/sunrise.conf -static_data=./server/webapp/ -listen=:6060 -grpc_listen=:6080 -cluster_self=one &
$GOPATH/bin/sunrise -config=./server/sunrise.conf -static_data=./server/webapp/ -listen=:6061 -grpc_listen=:6081 -cluster_self=two &
```
...
Sunrise does not provide ICE servers out of the box. You must install and configure (or purchase) your own servers otherwise video and voice calling will not be available.

Once you obtain the ICE TURN/STUN configuration from your service provider, add it to `sunrise.conf` section `"webrtc"` - `"ice_servers"` (or `"ice_servers_file"`). Also change `"webrtc"` - `"enabled"` to `true`. An example configuration is provided in the `sunrise.conf` for illustration only. IT WILL NOT FUNCTION because it uses dummy values instead of actual server addresses.
...
```
nohup $GOPATH/bin/server -config=./server/sunrise.conf -static_data=$HOME/sunrise/webapp/ &
exit
```
...
For more details see https://sunrise/chat/issues/25.
