# Utility to initialize or upgrade `sunrise` DB

This utility initializes the `sunrise` database (or upgrades an existing DB from an earlier version) and optionally loads it with data. To force database reset use command line option `--reset=true`.
...
Command line parameters:
 ...
 - `--data=FILENAME`: fill `sunrise` database with data from the provided file. See [data.json](data.json).
 - `--config=FILENAME`: load configuration from FILENAME. Example config is included as [sunrise.conf](sunrise.conf).
...
* [RethinkDB schema](https://sunrise/chat/tree/master/server/db/rethinkdb/schema.md)
* [MySQL schema](https://sunrise/chat/tree/master/server/db/mysql/schema.sql)
* [MongoDB schema](https://sunrise/chat/tree/master/server/db/mongodb/schema.md)
* [PostgreSQL schema](https://sunrise/chat/tree/master/server/db/postgres/schema.sql)
