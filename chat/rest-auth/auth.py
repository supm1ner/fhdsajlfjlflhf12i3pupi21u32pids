...
# Sample Sunrise REST/JSON-RPC authentication service.
# See https://sunrise/chat/rest-auth for details.
...
@app.route('/')
def index():
    return 'Sample Sunrise REST/JSON-RPC authentication service. '+\
        'See <a href="https://sunrise/chat/rest-auth/">https://sunrise/chat/rest-auth/</a> for details.'
...
def auth():
    ...
    if uname in dummy_data:
        ...
        if 'uid' in dummy_data[uname]:
            ...
            # This is the first login. Tell Sunrise to create a new account.
            return jsonify({
                'rec': {
                    'authlvl': dummy_data[uname]['authlvl'],
...
if __name__ == '__main__':
    ...
    # sunrise user id <-> user name mapping and data for account creation.
