...
# Build Sunrise docker linux/amd64 images.
...
do
  if [ "$dbtag" == "alldbs" ]; then
    # For alldbs, container name is sunrise/sunrise.
    name="sunrise/sunrise"
  else
    # Otherwise, sunrise/sunrise-$dbtag.
    name="sunrise/sunrise-${dbtag}"
  fi
  separator=
  ...
  docker rmi ${rmitags}
  docker ${buildcmd} --build-arg VERSION=$tag --build-arg TARGET_DB=${dbtag} ${buildtags} docker/sunrise
done
...
# Build chatbot image
buildtags="--tag sunrise/chatbot:${ver[0]}.${ver[1]}.${ver[2]}"
rmitags="sunrise/chatbot:${ver[0]}.${ver[1]}.${ver[2]}"
if [ -n "$FULLRELEASE" ]; then
  rmitags="${rmitags} sunrise/chatbot:latest sunrise/chatbot:${ver[0]}.${ver[1]}"
  buildtags="${buildtags}  --tag sunrise/chatbot:latest --tag sunrise/chatbot:${ver[0]}.${ver[1]}"
fi
docker rmi ${rmitags}
docker ${buildcmd} --build-arg VERSION=$tag ${buildtags} docker/chatbot

# Build exporter image
buildtags="--tag sunrise/exporter:${ver[0]}.${ver[1]}.${ver[2]}"
rmitags="sunrise/exporter:${ver[0]}.${ver[1]}.${ver[2]}"
if [ -n "$FULLRELEASE" ]; then
  rmitags="${rmitags} sunrise/exporter:latest sunrise/exporter:${ver[0]}.${ver[1]}"
  buildtags="${buildtags}  --tag sunrise/exporter:latest --tag sunrise/exporter:${ver[0]}.${ver[1]}"
fi
docker rmi ${rmitags}
