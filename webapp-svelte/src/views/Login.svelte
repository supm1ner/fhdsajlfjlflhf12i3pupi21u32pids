<script>
  ...
  import { connect, login } from '../lib/tinode.js';
  ...
  async function handleLogin() {
    ...
    try {
      ...
      const c = (await import('../lib/tinode.js')).getClient();
      appState.user = { id: c.myUID, name: loginVal };
      appState.chats = [];
...
<style>
  ...
  .login-card { width: 360px; display: flex; flex-direction: column; gap: 16px; padding: 32px; animation: fadeIn 400ms ease; }
