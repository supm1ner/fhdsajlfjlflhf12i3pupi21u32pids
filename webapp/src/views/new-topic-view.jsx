...
class NewTopicView extends React.Component {
  ...
  handleSearchContacts(query) {
    query = query.trim() || Sunrise.DEL_CHAR;
    if (!/[\s,:]/.test(query) && query != Sunrise.DEL_CHAR) {
      // No colons, spaces or commas, not DEL char. Try as email, phone, or alias.
      const email = asEmail(query);
      if (email) {
        query = `${Sunrise.TAG_EMAIL}${email}`;
      } else {
        const tel = asPhone(query);
        if (tel) {
          query = `${Sunrise.TAG_PHONE}${tel}`;
        } else {
          if (query[0] == '@') {
          ...
          query = `${Sunrise.TAG_ALIAS}${query},${query}`;
        }
      }
    }
    this.props.onSearchContacts(query);
    this.setState({searchQuery: Sunrise.isNullValue(query) ? null : query});
  }
    ...
    return (
      <div className="flex-column">
        ...
        this.state.tabSelected === 'byid' ?
          <NewTopicById
            myURI={Sunrise.URI_TOPIC_ID_PREFIX + this.props.sunrise.getCurrentUserID()}
