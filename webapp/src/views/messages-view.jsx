...
import { Drafty, Sunrise, TheCard } from 'sunrise-sdk';
...
class MessagesView extends React.Component {
  ...
  componentSetup(prevProps, prevState) {
    ...
    if (this.state.topic != prevState.topic) {
      if (prevState.topic && !Sunrise.isNewGroupTopicName(prevState.topic)) {
  ...
  handleDescChange(desc) {
    if (Sunrise.isSelfTopicName(this.props.topic)) {
      this.setState({
      ...
      this.readNotificationTimer = setInterval(_ => {
        ...
        while (this.readNotificationQueue.length > 0) {
          ...
          if (n.sendAt <= now) {
            ...
            this.readNotificationQueue.shift();
            if (n.seq == 0 || Sunrise.isServerAssignedSeq(n.seq)) {
  ...
  handleMessageUpdate(msg) {
    ...
    if (status >= Sunrise.MESSAGE_STATUS_SENT && msg.from != this.props.myUserId) {
      this.postReadNotification(msg.seq);
    }
  }
      ...
      if (topic.isSelfType()) {
        ...
        if (acs && acs.isDeleter()) {
          ...
          if (!canDelete) {
            ...
            const maxDelAge = this.props.sunrise.getServerParam(Sunrise.MSG_DELETE_AGE, 0) | 0;
  ...
  sendFileAttachment(file) {
    ...
    const maxInbandAttachmentSize = (this.props.sunrise.getServerParam('maxMessageSize',
    ...
    if (file.size > maxInbandAttachmentSize) {
      ...
      // Pass data and the uploader to the SunriseWeb.
  ...
  sendAudioAttachment(url, preview, duration) {
    fetch(url)
      ...
      .then(blob => {
        // Server-provided limit reduced for base64 encoding and overhead.
        const maxInbandAttachmentSize = this.props.sunrise.getServerParam('maxMessageSize', MAX_INBAND_ATTACHMENT_SIZE) * 0.75 - 1024;
        if (blob.size > maxInbandAttachmentSize) {
          // Too large to send inband - uploading out of band and sending as a link.
          const uploader = this.props.sunrise.getLargeFileHelper();
          if (!uploader) {
    ...
    if (!this.state.topic) {
      ...
      if (this.state.imagePreview) {
        ...
        if (isChannel) {
          ...
          if (cont) {
            if (Sunrise.isP2PTopicName(cont.topic)) {
              if (cont.online) {
            ...
            } else if (Sunrise.isGroupTopicName(cont.topic)) {
              if (topic.subcnt) {
