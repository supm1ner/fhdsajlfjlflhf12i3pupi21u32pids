...
class SunriseWeb extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    new Promise((resolve, reject) => {
      this.sunrise = SunriseWeb.tnSetup(this.state.serverAddress, isSecureConnection(), this.state.transport,
  ...
  static tnSetup(serverAddress, secureConnection, transport, locale, persistentCache, onSetupCompleted) {
    const sunrise = new Sunrise({appName: APP_NAME, host: serverAddress, apiKey: API_KEY, transport: transport,
      secure: secureConnection, persist: persistentCache}, onSetupCompleted);
    sunrise.setHumanLanguage(locale);
    sunrise.enableLogging(LOGGING_ENABLED, true);
    return sunrise;
  }

  // Sunrise received a push notification from the server.
  ...
  // Check if a newer version of SunriseWeb app is available at the server.
  ...
  handleHashRoute() {
    ...
    if (hash.path && hash.path.length > 0) {
      ...
      if (topicName != this.state.topicSelected) {
        if (!Sunrise.topicType(topicName)) {
    ...
    if (hash.params.cred_done) {
      Object.assign(newState,
        SunriseWeb.stateForError(this.props.intl.formatMessage(messages.cred_confirmed_successfully), 'info'));
    }
  ...
  handleLoginSuccessful() {
    ...
    if (LocalStorageUtil.getObject('keep-logged-in')) {
      LocalStorageUtil.setObject('auth-token', this.sunrise.getAuthToken());
    }
  ...
  static prepareSearchableContacts(chatList, foundContacts) {
    ...
    for (const c of chatList) {
      if (Sunrise.isP2PTopicName(c.topic)) {
          merged[c.topic] = {
            user: c.topic,
            updated: c.updated,
            public: c.public,
  ...
  handleNewAccountRequest(login_, password_, public_, cred_, tags_) {
    ...
    this.sunrise.connect(this.state.serverAddress)
      .then(_ => {
        let attachments;
        ...
        return this.sunrise.createAccountBasic(login_, password_,
          {public: public_, tags: tags_, cred: Sunrise.credential(cred_), attachments: attachments});
      }).then(ctrl => {
        if (ctrl.code >= 300 && ctrl.text == 'validate credentials') {
          SunriseWeb.navigateToCredentialsView(ctrl.params);
        } else {
  ...
  handleUpdateAccountTagsRequest(_, tags) {
    this.sunrise.getMeTopic().setMeta({tags: tags})
      .catch(err => this.handleError(err.message, 'err'));
  }
  ...
  handleGlobalSettings(settings) {
    ...
    this.sunrise = SunriseWeb.tnSetup(serverAddress, secureConnection, transport,
  ...
  handleCredConfirm(method, response) {
    SunriseWeb.navigateToCredentialsView({cred: [method], code: response});
  }
  ...
  handleStartTopicRequest(topicName, newTopicParams, isChannel) {
    ...
    const params = {};
    if (Sunrise.isP2PTopicName(topicName)) {
  ...
  handleTopicUpdateRequest(topicName, pub, priv, defacs) {
    ...
    if (topic) {
      ...
      if (pub) {
        if (pub.photo) {
          if (pub.photo.ref && pub.photo.ref != Sunrise.DEL_CHAR) {
            attachments = [pub.photo.ref];
          } else if (!pub.photo.data || pub.photo.data == Sunrise.DEL_CHAR) {
            pub.photo = Sunrise.DEL_CHAR;
          }
        }
        params.public = pub;
      }
      ...
      if (typeof priv == 'string') {
        params.private = (priv === Sunrise.DEL_CHAR) ?
          Sunrise.DEL_CHAR : {comment: priv};
      }
      if (defacs) {
  ...
  handleUpdatePasswordRequest(password)  {
    ...
    if (password) {
      this.sunrise.updateAccountBasic(null, this.sunrise.getCurrentLogin(), password)
        .catch(err => this.handleError(err.message, 'err'));
    }
  }
  ...
  handleLogout() {
    ...
    cleared.then(_ => {
      this.sunrise = SunriseWeb.tnSetup(this.state.serverAddress, isSecureConnection(),
  ...
  defaultTopicContextMenu(topicName) {
    ...
    p2p = Sunrise.isP2PTopicName(topicName);
    self = Sunrise.isSelfTopicName(topicName);
  ...
  handleValidateCredentialsRequest(cred, code, token) {
    if (this.sunrise.isAuthenticated()) {
      ...
      this.sunrise.getMeTopic().setMeta({cred: {meth: cred, resp: code}})
  ...
  handlePasswordResetRequest(method, value) {
    ...
    return this.sunrise.connect()
      .then(_ => this.sunrise.requestResetAuthSecret('basic', method, value))
      .catch(err => {
        // Socket error
        this.handleError(err.message, 'err');
      });
  ...
  handleResetPassword(newPassword, tempAuth) {
    ...
    if (!secret || !tempAuth.scheme) {
      ...
      this.sunrise.connect()
        .then(_ => this.sunrise.updateAccountBasic(null, null, newPassword, {scheme: tempAuth.scheme, secret: secret}))
        .then(_ => {
  ...
  handleInfoMessage(info) {
    ...
    switch (info.event) {
      case 'accept':
        ...
        if (Sunrise.isMeTopicName(info.topic) && this.sunrise.isMe(info.from)) {
          this.setState({
            callTopic: null,
            callState: CALL_STATE_NONE,
            callSeq: null,
  ...
  handleDataMessage(data) {
    if (data.head && data.head.webrtc && data.head.webrtc == CALL_HEAD_STARTED && Sunrise.isP2PTopicName(data.topic)) {
...
export default injectIntl(SunriseWeb);
