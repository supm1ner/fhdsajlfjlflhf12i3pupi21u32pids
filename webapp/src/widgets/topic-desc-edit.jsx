...
class TopicDescEdit extends React.Component {
  constructor(props) {
    ...
    const isSelf = Sunrise.isSelfTopicName(this.props.topic);
    this.state = {
      isMe: Sunrise.isMeTopicName(this.props.topic),
      isSelf: isSelf,
  ...
  handlePrivateUpdate(comment) {
    ...
    if (this.state.private !== comment) {
      ...
      this.props.onUpdateTopicDesc(this.props.topic, null, comment || Sunrise.DEL_CHAR);
    }
  }
  ...
  handleImageUpdated(mime, img) {
    ...
    if (!img) {
      ...
      this.props.onUpdateTopicDesc(this.props.topic, theCard(null, Sunrise.DEL_CHAR));
    }
  }
  ...
  handleAliasUpdate(alias) {
    alias = (alias || '').trim();
    const tags = alias ?
      Sunrise.setUniqueTag(this.state.tags, Sunrise.TAG_ALIAS + alias.toLowerCase()) :
      Sunrise.clearTagPrefix(this.state.tags, Sunrise.TAG_ALIAS);
  ...
  validateAlias(alias) {
    ...
    const valid = Sunrise.isValidTagValue(alias);
    if (!valid) {
    ...
    this.aliasCheckTimer = setTimeout(_ => {
      this.aliasCheckTimer = null;
      fnd.checkTagUniqueness(`${Sunrise.TAG_ALIAS}${alias}`, this.props.topic)
        .then(ok => {
