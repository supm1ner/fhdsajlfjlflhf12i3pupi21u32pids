...
export function theCard(fn, imageUrl, imageMimeType, note) {
  ...
  if (typeof note == 'string') {
    card = card || {};
    card.note = note ? note : Sunrise.DEL_CHAR;
  }
  ...
  if (imageUrl) {
    ...
    if (matches) {
      ...
      card.photo = {
        ...
        ref: Sunrise.DEL_CHAR
      };
    } else {
      card.photo = {
        data: Sunrise.DEL_CHAR,
        ref: imageUrl
      };
    }
...
// It will cause Sunrise server to add 'Content-Disposition: attachment' header when serving it.
...
export function deliveryMarker(received) {
  switch (received) {
    case Sunrise.MESSAGE_STATUS_SENDING:
      return { name: 'access_time' }; // watch face
    case Sunrise.MESSAGE_STATUS_FAILED:
    case Sunrise.MESSAGE_STATUS_FATAL:
      return { name: 'warning', color: 'danger-color' }; // yellow icon /!\
    case Sunrise.MESSAGE_STATUS_SENT:
      return { name: 'done' }; // checkmark
    case Sunrise.MESSAGE_STATUS_RECEIVED:
      return { name: 'done_all' }; // double checkmark
    case Sunrise.MESSAGE_STATUS_READ:
